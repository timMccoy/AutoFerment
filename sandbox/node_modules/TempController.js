...
TempController.prototype.load = function(scheduleFilePath) 
{
	if(this.state.mode == state.OFF || this.state.mode == state.STOPPED)
	{
		...
		try {
			...
			var stats = fs.lstatSync("../data/schedules/beerSchedule.xml");
			...
			if(stats.isFile())
			{
				...
				var data, etree;
	 
				data = fs.readFileSync('../data/schedules/beerSchedule.xml').toString();
				etree = et.parse(data);
...
TempController.prototype.freeLEDs = function() 
{
	console.log("***********int**************");
	this.fridgeController.freeLED();
	this.sensorController.freeLED();
}
